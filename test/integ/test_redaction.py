...
class PiiRedactionIntegTest(BasicIntegTest):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.lambda_function_arn = cls._create_function("pii_redaction", 'handler.redact_pii_documents_handler',
    ...
    def test_request_timeout(self):
        ...
        # To keep total integration test time low we avoid making multiple retry attempts
    ...
    def test_redaction_lambda_with_default_entity_types(self):
        start_time = datetime.now(tz=tzutc())
        ...
        with open(f"{self.DATA_PATH}/pii_output.txt") as expected_output_file:
        ...
        self._validate_api_call_latency_published(self.s3ol_access_point_arn, start_time, is_pii_detection_performed=True)

    def test_redaction_lambda_with_pii_overridden_entity_types(self):
        self._update_lambda_env_variables(self.lambda_function_arn,
                                          {"LOG_LEVEL": "DEBUG",
                                           "PII_ENTITY_TYPES": "CREDIT_DEBIT_NUMBER,BANK_ROUTING,BANK_ACCOUNT_NUMBER,EMAIL,PASSWORD"})
        start_time = datetime.now(tz=tzutc())
        response = self.s3_client.get_object(Bucket=self.s3ol_access_point_arn, Key='pii_input.txt')
        with open(f"{self.DATA_PATH}/pii_bank_routing_redacted.txt") as expected_output_file:
            assert response['Body'].read().decode('utf-8') == expected_output_file.read()
        self._validate_pii_count_metric_published(self.s3ol_access_point_arn, start_time,
                                                  ['BANK_ROUTING', 'CREDIT_DEBIT_NUMBER', 'BANK_ACCOUNT_NUMBER'])

    def test_redaction_lambda_fails_when_file_size_exceeds_limit(self):
    ...
    def test_redaction_lambda_succeeds_with_an_overridden_max_file_size_limit(self):
        self._update_lambda_env_variables(self.lambda_function_arn, {"LOG_LEVEL": "DEBUG", "DOCUMENT_MAX_SIZE": "1500000",
                                                                     "DOCUMENT_MAX_SIZE_CONTAINS_PII_ENTITIES": "5000"})
        response = self.s3_client.get_object(Bucket=self.s3ol_access_point_arn, Key='1mb_pii_text')
        with open(f"{self.DATA_PATH}/1mb_pii_redacted_text") as expected_output_file:
            assert response['Body'].read().decode('utf-8') == expected_output_file.read()

    def test_redaction_lambda_with_unsupported_file(self):
