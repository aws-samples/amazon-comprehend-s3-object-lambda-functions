...
with open(os.path.join('{}','..' ,'data', '1mb_text')) as textfile:
    text = " ".join(textfile.readlines())
segmenter = Segmenter(overlap_tokens=20, max_doc_size=5000)
        """.format(this_module_path)
        segmentation_time = timeit.timeit("segmenter.segment(text)", setup=setup, number=100)
    ...
    def test_redaction_with_replace_entity_type(self):
        text = "Hello Zhang Wei. Your AnyCompany Financial Services, LLC credit card account 1111-0000-1111-0000 has a minimum payment of $24.53"
        redactor = Redactor(RedactionConfig(pii_entity_types=['NAME'], mask_mode=REPLACE_WITH_PII_ENTITY_TYPE, confidence_threshold=0.1))
        redacted_text = redactor.redact(text, [{'Score': 0.234, 'Type': 'NAME', 'BeginOffset': 6, 'EndOffset': 15},
                                               {'Score': 0.765, 'Type': 'CREDIT_DEBIT_NUMBER', 'BeginOffset': 77, 'EndOffset': 96}])
